...
export default function RoomPage() {
    ...
    const createPeerConnection = (peerId: string) => {
        ...
        pc.ontrack = (e) => {
            ...
            if (!stream) {
                console.warn(`Received ontrack event from ${peerId} with no streams`, e);
                return;
            }
        ...
        pc.onicecandidate = (e) => {
            if (e.candidate) {
                send({ type: 'ice-candidate', candidate: e.candidate, targetId: peerId });
            }
        };
    ...
    const joinRoom = () => {
        ...
        const ws = new WebSocket(`${protocol}//${window.location.hostname}:4000/ws?room=${roomId}`);
        wsRef.current = ws;
    ...
    const stopScreenShare = () => {
        ...
        pcRefs.current.forEach(pc => {
            ...
            const sender = label ? senders.find(s => s.track?.kind === 'video' && s.track?.label === label) : undefined;
    ...
    if (isPreview) return <PreviewScreen micOn={micOn} videoOn={videoOn} toggleMic={toggleMic} toggleVideo={toggleVideo} onJoin={joinRoom} videoRef={localVideoRef} />;
    ...
    return (
        <div className="min-h-screen bg-gray-950 text-white flex flex-col">
            ...
            <div className="bg-gray-900 border-b border-gray-800 p-4 flex justify-between items-center">
                ...
                <button onClick={() => navigator.clipboard.writeText(window.location.href)} className="bg-gray-800 hover:bg-gray-700 px-4 py-2 rounded-lg text-sm transition">
                    Copy Link
                </button>
            </div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                    ...
                    {peers.map(peerId => {
                        ...
                        if (streams.length === 0) {
                            ...
                            return (
                                <div key={peerId} className="bg-gray-800 rounded-xl aspect-video flex items-center justify-center shadow-lg">
                                    <span className="text-gray-500">Peer {peerId.slice(0, 4)} joining...</span>
                                </div>
                            );
                        }
            ...
            <div className="bg-gray-900 border-t border-gray-800 p-6 flex justify-center gap-4">
                <ControlBtn onClick={toggleMic} active={micOn} onIcon="🎤" offIcon="🔇" />
                <ControlBtn onClick={toggleVideo} active={videoOn} onIcon="📹" offIcon="🚫" />
                <button
                ...
                <button onClick={() => window.location.reload()} className="px-6 py-4 rounded-full bg-red-600 hover:bg-red-700 font-bold shadow-lg">
                    Leave
                </button>
            </div>
        </div>
...
function VideoCard({ stream, muted, label, mirrored, isScreen, isOff }: any) {
    ...
    return (
        <div className={`relative bg-gray-800 rounded-xl overflow-hidden aspect-video shadow-xl ring-1 ring-gray-700 group`}>
            {stream && (
                <video
                    ref={ref}
                    autoPlay
    ...
    return (
        <div className="min-h-screen bg-black text-white flex items-center justify-center">
            <div className="text-center max-w-md p-6 bg-gray-900 border border-red-800 rounded-xl">
                ...
                <p className="text-gray-400 mb-4">You are already in this room. Please use the existing tab.</p>
                <button onClick={() => window.close()} className="bg-red-600 px-6 py-2 rounded-lg">Close Tab</button>
            </div>
        </div>
    );
}

function PreviewScreen({ micOn, videoOn, toggleMic, toggleVideo, onJoin, videoRef }: any) {
    return (
        <div className="min-h-screen bg-gray-950 text-white flex items-center justify-center p-4">
            <div className="max-w-lg w-full bg-gray-900 p-8 rounded-2xl shadow-2xl border border-gray-800">
                ...
                <div className="relative aspect-video bg-black rounded-xl overflow-hidden mb-8 ring-2 ring-blue-500/30">
                    <video ref={videoRef} autoPlay muted playsInline className={`w-full h-full object-cover transform -scale-x-100 ${!videoOn && 'hidden'}`} />
                    {!videoOn && <div className="absolute inset-0 flex items-center justify-center text-gray-500">Camera Off</div>}
                </div>
                <div className="flex justify-center gap-6 mb-8">
                    <ControlBtn onClick={toggleMic} active={micOn} onIcon="🎤" offIcon="🔇" />
                    <ControlBtn onClick={toggleVideo} active={videoOn} onIcon="📹" offIcon="🚫" />
                </div>
                <button onClick={onJoin} className="w-full bg-blue-600 hover:bg-blue-700 py-4 rounded-xl font-bold text-lg transition">
                    Enter Meeting
                </button>
            </div>
        </div>
